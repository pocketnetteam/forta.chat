...
- [x] **0.1 Продуктовые решения** — закрыть открытые вопросы плана (§9, пункты 1, 2, 3, 4, 6) хотя бы
...
- [x] **0.2 Собрать `local-ai` локально** — `pnpm install && pnpm build` в `C:\inetpub2026\localai`,
  подтвердить `dist/` собирается для всех трёх subpath-экспортов (`.`, `./adapters/capacitor`,
  `./adapters/node-testing`).
- [x] **0.3 Подключить как `file:` зависимость** — `npm install file:../../inetpub2026/localai` в
...
- [x] **0.4 Замер размера APK** — release-сборка до/после добавления нативных плагинов, зафиксировать
...
**Статус (2026-08-11):** 0.1–0.4 закрыты (см. `decisions.md`). 0.5/0.6 **сознательно отложены** — нет
доступа к реальному/эмулированному Android-устройству в этой сессии. Фаза формально не закрыта до
их выполнения — см. `qa-checklist-phase7.md`, который включает 0.5/0.6 в общий ручной прогон.
...
- [x] **1.1 `LocalAiChat`/`LocalAiMessage` типы** — `src/shared/lib/local-db/schema.ts`, `version(18)`
  (копирует все существующие таблицы без изменений + добавляет `aiChats`/`aiMessages`, план §3).
- [x] **1.2 `AiChatRepository`** — `src/shared/lib/local-db/ai-chat-repository.ts` — `getAll`/`get`/
...
- [x] **1.3 `AiMessageRepository`** — `src/shared/lib/local-db/ai-message-repository.ts` —
  `listByChat(chatId)`/`create`/`updateContent`/`updateStatus`/`deleteByChat` (каскад при удалении
  чата). Unit-тесты.
- [x] **1.4 Каскадное удаление** — `AiChatRepository.delete(id)` вызывает
...
- [x] **2.1 Сборка портов** — `src/entities/local-ai/lib/create-client.ts` — собирает `LocalAiPorts` из
...
- [x] **2.2 `useLocalAiStore`** — `src/entities/local-ai/model/local-ai-store.ts` — состояние:
...
- [x] **2.3 Подписка на события `local-ai`** — `client.on('download:progress', ...)`,
...
- [x] **2.4 Lifecycle-хуки** — `releaseRuntime()` на logout и на переключение аккаунта (симметрично
...
- [x] **3.1 `useAiChatStore`** — `src/entities/ai-chat/model/ai-chat-store.ts` — `chats` через
  `useLiveQuery(() => aiChatRepository.getAll())`, `activeChatId`, `messagesByChat` (аналог
  `useLiveQuery` per-chat).
- [x] **3.2 `createChat(title?)`** — пишет в Dexie немедленно, затем (если клиент уже создан и модель
...
- [x] **3.3 `sendMessage(chatId, text)`** — Mode B поток (план §2): пишет user-сообщение в Dexie
...
- [x] **3.4 Отмена генерации** — `cancelMessage(chatId)` дёргает `AbortSignal`, ожидаемый результат —
...
- [x] **3.5 `RuntimeBusyError` UX** — `isGenerating` в `useLocalAiStore` блокирует `sendMessage` во всех
  AI-чатах одновременно, не только в активном (план §7.3). Тест на прямую попытку двух параллельных
  `sendMessage`.
- [x] **3.6 `renameChat`/`deleteChat`** — `renameChat` → Dexie + `client.upsertChat({id, title})`;
  `deleteChat` → Dexie каскад (Фаза 1.4) + `client.deleteChat(id)`.
- [x] **3.7 Интеграционный тест Mode B** — вручную (или через `local-ai`'s node-testing адаптеры)
...
- [x] **4.1 `activeFilter` += `"ai"`** — `ChatSidebar.vue`, `visibleTabValues` (план §7.1: видимость по
  `isNative`, не по наличию данных — в отличие от `channels`).
  `<template #ai><AiChatList ... /></template>` в `SwipeableTabs`.
- [x] **4.2 `AiChatList.vue`** — `src/features/ai-chat/ui/AiChatList.vue`, по образцу `ChannelList.vue`,
  список из `useAiChatStore().chats`, пустое состояние («ещё нет AI-чатов»).
- [x] **4.3 Контекстная кнопка «новый чат»** — `ChatSidebar.vue` шапка, `v-if="activeFilter === 'ai'"`
  (план §7.2), `handleNewAiChat` → `createChat()` → открыть чат.
- [x] **4.4 i18n-ключи** — `ai.newChat`, `ai.emptyState`, `ai.tabLabel` и т.д. во всех языковых файлах
  проекта (сверить со списком существующих локалей).
- [x] **4.5 Android back-button** — если открыт AI-чат, аппаратная кнопка «назад» должна вести себя как
...
- [x] **5.1 `AiModelGate.vue`** — общий виджет «модель не готова» (план §7): три состояния — не скачана
...
- [x] **5.2 `AiChatView.vue`** — список сообщений (переиспользовать существующий виртуальный
...
- [x] **5.3 Composer-интеграция** — переиспользовать существующий компонент ввода из
...
- [x] **5.4 In-chat статус-баннер** — над списком сообщений: если `downloadState.model` активен —
  прогресс; если `!modelReady` — CTA на скачивание (использует `AiModelGate.vue` из 5.1, не дублирует
  логику, план §4.1 п.2).
- [x] **5.5 Обработка ошибок генерации** — `status: 'error'` сообщение рендерится отдельным стилем
...
**Статус (2026-08-11):** 5.1–5.5 реализованы (`AiModelGate.vue`, `AiComposer.vue`, `AiChatView.vue`,
`ChatPage.vue`-вайринг), `vue-tsc`/`vitest` зелёные. Composer — **новый минимальный компонент**, не
`MessageInput.vue` (см. код-комментарий в `AiComposer.vue`): `MessageInput.vue` — 1300+ строк,
жёстко завязан на Matrix-отправку/медиа/голос/опросы без колбэк-параметра для подмены получателя, и
находится под активной параллельной задачей по клавиатуре (`CLAUDE.md`) — риск конфликта от
добавления туда seam выше цены нового маленького компонента. Клавиатурная безопасность не
дублируется: `AiComposer.vue` использует тот же глобальный класс `safe-bottom`/`--keyboardheight`,
что и `ChatWindow.vue`. Ручной прогон на устройстве не выполнен — нет доступа.
...
- [x] **6.1 `SettingsSubView` += `'localAi'`** — `use-sidebar-tab.ts`.
- [x] **6.2 Кнопка в `SettingsPanel.vue`** — `v-if="isNative"`, по образцу существующих пунктов меню,
  между Storage и Networking (план §8).
- [x] **6.3 `LocalAiSettingsSection.vue`** — `src/features/settings/ui/LocalAiSettingsSection.vue`, по
  ...
  (`refreshManifest()`).
- [x] **6.4 Роутинг в `SettingsContentPanel.vue`** — добавить ветку `settingsSubView === 'localAi'`.
- [x] **6.5 «Удалить модель»** — решить открытый вопрос плана §9 п.7 (нужен ли API-метод в `local-ai`,
...
- [x] **7.1 `autoUnloadOnBackground`** — решить, включать ли (`LocalAiConfig.autoUnloadOnBackground`) —
  компромисс память/задержка возврата (ТЗ §11.2); задокументировать выбор в плане, не оставлять
  implicit default.
- [x] **7.2 `releaseRuntime()` на logout/переключение аккаунта** — подтвердить Фазой 2.4, добавить
...
- [x] **7.3 Tor bypass для загрузки модели** — подтвердить (план §10), что `@capgo/capacitor-downloader`
...
- [x] **7.5 Полная верификация перед PR** — `npm run build` ✅, `npx vue-tsc --noEmit` ✅, `npm run test`
  ✅ (363 файлов, только 5 pre-existing несвязанных падения — см. README.md). `npm run lint` **не
  существует в этом репозитории** (нет ESLint-конфига/скрипта в `package.json` — сверено, не
  пропущено). `review-team`/`review-fix` skills недоступны в этом окружении (см. отчёт сессии) —
  использован `code-review` на высоком уровне вместо.
- [x] **7.6 Логи `local-ai` → существующий бажрепорт** (план §11) — прикладывать
...
**Статус (2026-08-11):** 7.1 (решение задокументировано в `decisions.md`), 7.2 (код + автоматическая
регрессия покрывающая гонку teardown/generation — см. `ai-chat-store.test.ts`, ручной сценарий на
устройстве не выполнен), 7.3 (закрыт чтением исходников плагина — см. `decisions.md`, конкретный
вывод, не «нужен девайс»), 7.6 (код готов) — сделаны. 7.4 **отложена** — см.
`qa-checklist-phase7.md`, требует устройства. 7.5 — см. итоговый отчёт сессии в `README.md`.
