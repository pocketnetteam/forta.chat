import type { CallLinkInfo, CallProviderKind } from "@/entities/chat/model/types";
...
const VALID_PROVIDERS: readonly CallProviderKind[] = ["zoom", "google_meet", "jitsi", "custom"];
 ...
 * field so a malformed/hostile body degrades to plain text instead of
...
export function parseCallLinkBody(body: string): CallLinkInfo | null {
  ...
  try {
    ...
    if (kind !== "voice" && kind !== "video") return null;
    if (typeof provider !== "string" || !VALID_PROVIDERS.includes(provider as CallProviderKind)) {
      return null;
    }
    return {
      provider: provider as CallProviderKind,
      kind,
      url,
