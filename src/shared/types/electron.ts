...
/** Auto-update lifecycle from main (`electron/auto-updater.cjs`). */
...
export interface ElectronUpdateState {
  ...
  percent: number | null;
  error: string | null;
  /** False in Vite/dev or when FORTA_DISABLE_AUTO_UPDATE=1. */
  enabled: boolean;
}
...
export interface ElectronAPI {
  ...
  getUpdateStatus: () => Promise<ElectronUpdateState>;
  checkForUpdates: () => Promise<ElectronUpdateState>;
  quitAndInstallUpdate: () => Promise<boolean>;
  onUpdateStatus: (cb: (state: ElectronUpdateState) => void) => () => void;
}
